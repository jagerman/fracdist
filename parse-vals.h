...
inline unsigned int parse_uint(const char *arg, unsigned int *result) {
...
inline bool parse_bool(const char *arg, bool *result) {
...
#define PARSE_Q_B_C \
        ...
        if (fail || q < 1 || q > fracdist_q_length)\
            return ERROR("Invalid q value ``%s''", argv[1]);\
\
        fail = parse_double(argv[2], &b);\
        if (fail || b < fracdist_b_min || b > fracdist_b_max)\
